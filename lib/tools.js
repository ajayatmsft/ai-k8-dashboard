...
function buildToolRegistry(ctx) {
  ...
  const tools = [
    ...
    // --- platform / supply-chain inventory (read-only) -----------------------
    {
      name: 'getHelmReleases',
      description: 'List Helm releases installed in the cluster (name, namespace, revision, status, chart, appVersion). Uses the helm binary when present, otherwise reads helm release Secrets. Use to see what was deployed via Helm.',
      parameters: { type: 'object', properties: { namespace: { type: 'string', description: 'Namespace, or omit for all.' } } },
      async handler({ namespace } = {}) {
        if (!listHelmReleases) return { error: 'helm discovery unavailable', items: [] };
        return listHelmReleases({ ns: namespace });
      },
    },
    {
      name: 'getClusterAddons',
      description: 'Detect cluster add-ons / operators installed (ingress, cert-manager, service mesh, CNI, observability, policy, identity, CSI drivers, GitOps) by correlating CRD API groups and system-namespace workloads.',
      parameters: { type: 'object', properties: {}, additionalProperties: false },
      async handler() {
        if (!detectAddons) return { error: 'addon detection unavailable', detected: [] };
        return detectAddons();
      },
    },
    {
      name: 'getServiceAccounts',
      description: 'List service accounts (namespace, name, token automount, attached secrets, and any cloud identity annotations such as Azure workload identity client-id, AWS role ARN, or GCP service account).',
      parameters: { type: 'object', properties: { namespace: { type: 'string', description: 'Namespace, or omit for all.' } } },
      async handler({ namespace } = {}) {
        if (!listServiceAccounts) return { error: 'service account listing unavailable', items: [] };
        return listServiceAccounts({ ns: namespace });
      },
    },
    {
      name: 'getPodIdentities',
      description: 'List managed identities attached to pods: Azure Workload Identity (annotated service accounts + pods opting in) and legacy AAD Pod Identity (AzureIdentity CRDs + aadpodidbinding labels).',
      parameters: { type: 'object', properties: { namespace: { type: 'string', description: 'Namespace, or omit for all.' } } },
      async handler({ namespace } = {}) {
        if (!listPodIdentities) return { error: 'identity discovery unavailable' };
        return listPodIdentities({ ns: namespace });
      },
    },
